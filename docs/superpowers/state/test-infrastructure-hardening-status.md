...
### Decisions Captured at Bootstrap
...
- **S (mock reduction):** SHIPPED 2026-05-03. Four PRs landed (#147, #148, #150, #151); see Phase 4-S below for the close-out summary.
...
- **U (story `play()` interactions):** SHIPPED 2026-05-06. Spec landed at `docs/superpowers/specs/2026-05-04-cluster-u-story-play-interactions-design.md`. Stood up the docs-side storybook-vitest integration (`docs/vitest.storybook.config.ts`), the CI lane (`story-interactions` job in `.github/workflows/ci.yml`), and `play: async` interactions on 27 stories (≥20 floor cleared) under `docs/src/stories/` covering overlays, forms, triggers, disclosure, display-heavy components, and other surfaces. Lane scoped via the `interaction` story tag so pre-existing render failures on untouched stories stay out of scope. `pnpm test:all` chains `pnpm test:stories`. Three PRs landed: #161 (initial 22 stories), #162 (`server.warmup` fix for cold-cache races), #167 (5 display-heavy components: ButtonGroup, ColorPicker, ProgressBar, SplitPanel, Tag).
