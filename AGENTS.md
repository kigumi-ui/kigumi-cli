...
| `scripts/is-entry-point.ts`              | `isEntryPoint(import.meta.url)`: true only when the script is the process entry point. Compares real paths, so an absolute invocation through a symlinked directory still runs `main()` instead of exiting 0 silently. Used by the parser and the React/Angular generators (issue #106)                                                                                                                                                           |
...
| `scripts/validate-cem-sync.ts`           | `validate:cem-sync`. Two halves, reported separately: component presence (committed `COMPONENT_METADATA` vs registry, always runs) and the manifest half (needs a complete CEM): prop-value drift (registry enums vs CEM attribute types) and attribute-name drift (`checkAttributeDrift()`, issue #100). Only a run where both halves were verified prints a pass (issue #43, `docs/adr/0003`). The manifest half runs in CI's `freshness` job   |
...
| `scripts/upstream-holds.json`            | Toolchain majors (`typescript`, `vitest`) already evaluated and deliberately held; `check-upstream-versions.ts` reads it so the weekly report stops re-flagging a decision already made, and only surfaces a genuinely new major                                                                                                                                                                                                                  |
