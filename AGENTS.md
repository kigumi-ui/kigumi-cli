...
**Version**: 1.1.0 | **Stack**: TypeScript, Commander, Zod
...
| `scripts/is-entry-point.ts`              | `isEntryPoint(import.meta.url)`: true only when the script is the process entry point. Compares real paths, so an absolute invocation through a symlinked directory still runs `main()` instead of exiting 0 silently. Every `.ts` script guards `main()` with it; never hand-roll an `argv[1]` compare. The three `.mjs` scripts run under plain `node`, cannot import it, and call `main()` unconditionally, so never import them (issue #106)  |
...
| `scripts/validate-cem-sync.ts`           | `validate:cem-sync`. Two halves, reported separately: component presence (committed `COMPONENT_METADATA` vs registry, always runs) and prop-value drift (registry enums vs the CEM's attribute types, needs a complete manifest). Only a run where both were verified prints a pass (issue #43, `docs/adr/0003`). The prop-value half runs in CI's `freshness` job                                                                                |
...
- every script under scripts/ now guards main() with isEntryPoint; 16 validators exited 0 silently when invoked by absolute path through a symlinked directory, issue #106
- is-entry-point row no longer claims every main() is guarded: eight scripts still call main() unconditionally and are unsafe to import, issue #106
- the five .ts scripts that called main() unconditionally (generate-skill-references, update-starter-snapshots, validate-templates, sync-wa-pro-shim, verify-test-app) are now guarded too; importing them no longer runs them, issue #106
