...
| `scripts/is-entry-point.ts`              | `isEntryPoint(import.meta.url)`: true only when the script is the process entry point. Compares real paths, so an absolute invocation through a symlinked directory still runs `main()` instead of exiting 0 silently. Scripts that guard `main()` must use it, never a hand-rolled `argv[1]` compare. Eight scripts still call `main()` unconditionally and must not be imported (issue #106)                                                    |
