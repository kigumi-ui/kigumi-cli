...
`label` `hint` `orientation` (horizontal|vertical) `required` `size`
...
`label` `hint` `value` `min` `max` `locale` `required` `disabled` `readonly` `name` `size` `appearance`
...
<Checkbox
  @change="(e: Event) => checked = (e.target as HTMLInputElement).checked"
/>
