...
- **Generated by script:** `scripts/generate-angular-templates.ts` (do not hand-edit). Run `pnpm generate:angular` after the script or `src/utils/component-metadata.ts` changes.
...
- **`on*` attributes:** Angular refuses any `on*` binding as an event handler, `[attr.once]` included, and the Template then fails to compile. The generator writes such a prop from `ngOnChanges` with `toggleAttribute` instead (IntersectionObserver's `once`), and refuses a non-boolean `on*` prop rather than emit a binding that cannot compile (issue #77)
...
```html
<!-- Angular -->
<k-dialog [open]="isOpen" (afterHideEvent)="isOpen = false">…</k-dialog>
```
