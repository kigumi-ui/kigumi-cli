...
/**
 * Web Awesome Pro components whose documentation prose must not be committed.
 *
 * Pro is commercial software: its license permits a seat holder to use it but
 * forbids handing Pro assets to people without a license. Names, types and
 * signatures are the interface this CLI generates against and stay; the prose
 * describing them is Fonticons' authored documentation and is dropped. Pro
 * licensees still get the real descriptions from the package in their editor.
 */
...
/**
 * Return a copy of the metadata with Pro components' descriptions removed.
 * Free-tier entries pass through untouched: that package is MIT licensed.
 */
function stripProDescriptions(
  metadata: Record<string, ComponentMetadata>
): Record<string, ComponentMetadata> {
  const result: Record<string, ComponentMetadata> = {};

  for (const [key, component] of Object.entries(metadata)) {
    if (!PRO_COMPONENTS.has(key)) {
    ...
    result[key] = {
      ...component,
      events: component.events.map(({ description: _drop, ...rest }) => rest),
      slots: component.slots.map(({ description: _drop, ...rest }) => rest),
      methods: component.methods.map(({ description: _drop, ...rest }) => rest),
    };
  }
