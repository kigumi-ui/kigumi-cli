...
describe('kigumi init', () => {
  ...
  describe('file generation', () => {
    it('generates required files for React + Vite project', async () => {
      ...
      expect(await fileExists(testDir, 'src/lib/webawesome.ts')).toBe(true);
  ...
  describe('file preservation', () => {
    ...
    it('regenerates webawesome.ts on re-init', async () => {
      testDir = await createTempProject('react-vite');

      // First init
      await runKigumi(testDir, ['init', '--no-install', '-y']);

      // Modify webawesome.ts
      await writeFile(
        testDir,
      ...
      const content = await readFile(testDir, 'src/lib/webawesome.ts');
