...
The three-way test is one exported predicate, `isAllowlistedAttribute(regKey, attr, policy = SHIPPED_ATTRIBUTE_POLICY)`. The Angular function harness calls it too: an allowlisted attribute is the only CEM attribute an Angular Template may leave without an `@Input()`, because Angular has no rest spread and cannot pass it through the way the React and Vue Templates do (issue #77). Allowlisting an attribute therefore also exempts it from the Angular attribute contract.
...
- `scripts/validate-cem-sync.ts` exports `isAllowlistedAttribute` and `SHIPPED_ATTRIBUTE_POLICY`: the drift check and the Angular function harness now share one "needs no registry prop" rule, issue #77
